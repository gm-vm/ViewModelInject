...
class WorkerInjectProcessor : AbstractProcessor() {
    ...
    override fun init(env: ProcessingEnvironment) {
        ...
        listenableWorkerType = elements.getTypeElement("androidx.work.ListenableWorker").asType()
    }
    ...
    private lateinit var listenableWorkerType: TypeMirror
    ...
    override fun process(annotations: Set<TypeElement>, roundEnv: RoundEnvironment): Boolean {
        ...
        // Wait unitl processing is ending to validate that the @WorkerModule's @Module annotation
        ...
        if (roundEnv.processingOver()) {
            ...
            if (userModuleFqcn != null) {
                ...
                if (!referencesGeneratedModule) {
                    error("@WorkerModule's @Module must include ${generatedModuleName.simpleName()}",
                        userModule)
                }
            }
        }
    ...
    private fun TypeElement.toWorkerInjectElementsOrNull(): WorkerInjectElements? {
        ...
        if (!types.isSubtype(asType(), listenableWorkerType)) {
            error("@WorkerInject-using types must be subtypes of androidx.work.ListenableWorker", this)
            valid = false
        }
        ...
        if (PRIVATE in constructor.modifiers) {
            error("@WorkerInject constructor must not be private.", constructor)
            return null
        }
     ...
     * - Unqualified assisted parameters of Context and WorkerParameters
    ...
    private fun TypeElement.toWorkerModuleElementsOrNull(
        ...
        if (!hasAnnotation("dagger.Module")) {
            error("@WorkerModule must also be annotated as a Dagger @Module", this)
            return null
        }
...
private val FACTORY = ClassName.get("com.vikingsen.inject.work", "WorkFactory")
private val FACTORY_KEYS = listOf(
    NamedKey(Key(ClassName.get("android.content", "Context")), "context"),
    NamedKey(Key(ClassName.get("androidx.work", "WorkerParameters")), "workerParameters")
)
